...
import { FiX, FiCheck, FiSave, FiInfo } from "react-icons/fi";
...
const EditAddressDetails = ({ close, data }) => {
  const { register, handleSubmit, reset, watch, setValue, formState: { errors } } = useForm({
    defaultValues: {
      ...
      stateCode: State.getStatesOfCountry('IN').find(s => s.name === data.state)?.isoCode || ""
    }
  });
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      const response = await Axios({
        ...
        data: {
            ...formData,
            address_line: formData.address_line_1 // Map back to address_line for API consistency if needed, 
            // but controller was updated to handle both or renamed. 
            // I'll keep it consistent with the backend changes.
        }
      });
    ...
    <section className='fixed inset-0 z-[2000] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4'>
      <div className='bg-white w-full max-w-2xl max-h-[90vh] rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 flex flex-col'>
        ...
        <div className='px-8 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50'>
          <div>
            <h2 className='font-bold text-xl text-slate-800'>Edit Shipping Address</h2>
            <p className='text-xs text-slate-400 mt-0.5'>Update your delivery information</p>
          </div>
