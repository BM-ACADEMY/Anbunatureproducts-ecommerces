...
const EditProductAdmin = ({ close, data: propsData, fetchProductData }) => {
  const [data, setData] = useState({
    ...
    category: Array.isArray(propsData.category) && propsData.category.length > 0 
      ? (typeof propsData.category[0] === 'object' ? propsData.category[0]._id : propsData.category[0]) 
      : '',
  ...
  const [selectedCategoryDetails, setSelectedCategoryDetails] = useState(
    Array.isArray(propsData.category) && propsData.category.length > 0 && typeof propsData.category[0] === 'object' 
      ? propsData.category[0] 
      : null
  );
  ...
  const handleRenameGroupName = (oldName) => {
    ...
    if (data.attributes.some((attr) => attr.name.toLowerCase() === groupRenameValue.trim().toLowerCase() && attr.name !== oldName)) {
      toast.error(`Attribute group "${groupRenameValue}" already exists.`);
      return;
    }
    setData((prev) => ({
      ...prev,
                  ...
                  <div className='flex flex-wrap gap-2 pt-1'>
                    {selectedCategoryDetails && (
                      <span className='bg-indigo-50 text-indigo-700 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-2 border border-indigo-100'>
                         {selectedCategoryDetails.name} <IoClose className='cursor-pointer' onClick={handleRemoveCategory} />
                      </span>
                    )}
                  </div>
               </div>
                        ...
                        {editingGroupName === attr.name ? (
                          <div className='flex items-center gap-2 flex-grow mr-4'>
                            <input 
                              ...
                              className='bg-white px-3 py-1 border border-[#279d68]/40 rounded outline-none text-sm font-bold w-full focus:ring-1 focus:ring-[#279d68]'
                              autoFocus
                            />
                            <button 
                              type='button' 
                              onClick={() => handleRenameGroupName(attr.name)}
                              className='bg-[#279d68] text-white px-3 py-1 rounded text-xs font-bold'
                            >
                              Save
                            </button>
                            <button 
                          ...
                          <div className='flex items-center gap-2'>
                            ...
                            <button 
                              ...
                              className='text-[#279d68] hover:text-green-700 p-1 rounded-lg transition-colors'
