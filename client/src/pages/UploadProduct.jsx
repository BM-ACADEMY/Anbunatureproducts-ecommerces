...
const UploadProduct = () => {
  ...
  useEffect(() => {
    console.log('Current reviews in state:', data.reviews);
  }, [data.reviews]);
  ...
  const handleAddReview = () => {
    ...
    setData((prev) => {
      ...
      console.log('Added review, new reviews array:', updatedData.reviews);
      return updatedData;
    });
  ...
  const handleDeleteReview = (indexToDelete) => {
    setData((prev) => {
      const updatedReviews = prev.reviews.filter((_, index) => index !== indexToDelete);
      console.log('Deleted review, new reviews array:', updatedReviews);
      return {
        ...prev,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const productDataToSend = {
        ...
        comboOffer: data.comboOffer, // Include comboOffer
      };
      ...
      if (responseData.success) {
        ...
        setData({
          ...
          attributes: [],
          reviews: [],
          comboOffer: false, // Reset comboOffer
        });
        setSelectCategory('');
    ...
    <Box sx={{ p: 2 }}>
      ...
      <Box sx={{ p: 3 }}>
        <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '16px' }}>
          ...
          <Divider sx={{ my: 2 }} />
          <Typography variant="h6" fontWeight="medium">More Details Section</Typography>
          {Object.keys(data.more_details).map((k) => (
            <Box key={k} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <IconButton
                onClick={() => {
                  const newMoreDetails = { ...data.more_details };
                  delete newMoreDetails[k];
                  setData((prev) => ({ ...prev, more_details: newMoreDetails }));
                }}
                color="error"
              >
                <DeleteIcon />
          ...
          <Divider sx={{ my: 2 }} />
          <Typography variant="h6" fontWeight="medium">Product (Size or quantity)</Typography>

          {data.attributes.map((attributeGroup) => (
            <Box key={attributeGroup.name} sx={{ border: '1px solid #ccc', p: 2, borderRadius: 1 }}>
          ...
          >
            {submitLoading ? <CircularProgress size={24} /> : 'Submit Product'}
          </Button>
        </form>
      </Box>
